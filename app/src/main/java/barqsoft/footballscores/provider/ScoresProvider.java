...
public class ScoresProvider extends ContentProvider
{
    ...
    private int match_uri(Uri uri)
    {
        if (BuildConfig.DEBUG) {
            Timber.d("match_uri(Uri uri)");
        }
        String link = uri.toString();
        if (BuildConfig.DEBUG) {
            Timber.d("link = uri.toString() is " + link);
            Timber.d("DatabaseContract.BASE_CONTENT_URI.toString() is " + DatabaseContract.BASE_CONTENT_URI.toString());
            Timber.d("DatabaseContract.scores_table.buildScoreWithDate().toString() is " + DatabaseContract.scores_table.buildScoreWithDate().toString());
            Timber.d("DatabaseContract.scores_table.buildScoreWithId().toString() is " + DatabaseContract.scores_table.buildScoreWithId().toString());
            Timber.d("DatabaseContract.scores_table.buildScoreWithLeague().toString() is " + DatabaseContract.scores_table.buildScoreWithLeague().toString());
        }
        {
    ...
    {
        if (BuildConfig.DEBUG) {
            Timber.d("getType(Uri uri)");
        }
        final int match = muriMatcher.match(uri);
        if (BuildConfig.DEBUG) {
            Timber.d("match = muriMatcher.match(uri) is " + match);
        }
        switch (match) {
            case MATCHES:
    ...
    {
        if (BuildConfig.DEBUG) {
            Timber.d("query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder)");
        }
        Cursor retCursor;
        //Log.v(FetchScoreTask.LOG_TAG,uri.getPathSegments().toString());
        int match = match_uri(uri);
        if (BuildConfig.DEBUG) {
            Timber.d("match_uri(uri) is " + match_uri(uri));
        }
        switch (match)
        {
